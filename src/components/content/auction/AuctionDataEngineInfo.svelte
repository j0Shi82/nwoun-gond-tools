...
$: hasPetreons = patreonData.reduce((aggr, cur) => {
...
<div class="mb-2">
  ...
  >
    <div class="flex items-center justify-center">
      {#if crawlActive}
      <Icon data="{faCircleCheck}" scale="{2}" class="text-green-700 mr-2 flex-none w-8"></Icon>
      <div>{@html $localize('auction.engine.up.data', localizeOptions)}</div>
      {/if}
      {#if crawlInactive}
      <Icon data="{faBan}" scale="{2}" class="text-red-700 mr-2 flex-none w-8"></Icon>
      <div>{@html $localize('auction.engine.down.data', localizeOptions)}</div>
      {/if}
      {#if fetching}
      <Spinner scaleClasses="h-8 w-8 flex-0" style="width: 2rem; margin-right: 0.5rem;" />
    ...
    <div class="patreons">
      {#each patreonData as tier, i}
        {#if tier.members.length}<div class="tier tier-{i}">{tier.members.map((member) => member.name).sort().join(', ')}</div>{/if}
      {/each}
    </div>
    <div class="text-center w-full italic">{ $localize('auction.engine.patreons.mention') }</div>
    {/if}
  </div>
</div>
