<script>
    ...
    function getDevPosts() {
        ...
        loading = true;
        makeApiCall({
            type: "devtracker/postlist",
            params: { curPage, curDev, curID },
            returnData: false,
        })
            .then((response) => {
    ...
    {#if apiData.length && !firstLoading}
        <div id="form" class="mt-2">
            <select
                bind:value={curDev}
                disabled={curID !== "0"}
                class="block w-full form-select bg-gray-300 border-nwoun border rounded-md bg-opacity-70 font-bold text-black h-12"
                id="grid-state"
            >
                <option value="" selected>-- Developer --</option>
                {#each devData as data}
                    <option value={data.dev_name}
                        >{data.dev_name} ({data.post_count})</option
                    >
                {/each}
            </select>
            <select
                bind:value={curID}
                disabled={curDev !== ""}
                class="block w-full form-select bg-gray-300 border-nwoun border rounded-md bg-opacity-70 font-bold text-black mt-2 h-12"
                id="grid-state"
            >
                <option value="0" selected>-- Hot Topics --</option>
                {#each topicData as data}
                    <option value={data.discussion_id}
                        >{data.discussion_name} ({data.post_count})</option
                    >
                {/each}
            </select>
        </div>
        {#each apiData as data}
            <DevtrackerPost
                postData={data}
                avatarSrc={avatarData[`${data.dev_name}::${data.dev_id}`]
                    ? avatarData[`${data.dev_name}::${data.dev_id}`]
                    : null}
            />
        {/each}
        {#if !finished}
