...
<div class="fixed inset-0 transition-opacity z-50 overflow-y-auto">
    <div bind:this="{background}" class="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0 inset-0 bg-gray-600 bg-opacity-75" on:click="{handleOuterClick}">
        <slot></slot>
    </div>
</div>
{/if}