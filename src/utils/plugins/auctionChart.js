        ...
        labels: {
          ...
          generateLabels(chart) {
            ...
            return Array.isArray(data.datasets) ? data.datasets.map((dataset, i) => ({
              text: `${dataset.label} (Min: ${Math.min.apply(null, dataset.data.map((d) => d.y)).toLocaleString()} - Max: ${Math.max.apply(null, dataset.data.map((d) => d.y)).toLocaleString()})`,
...
export default (domId, datasets, colorIdent) => {
  ...
  if (colors[colorIdent]) {
    ...
    curConfig.data.datasets[0].backgroundColor = colors[colorIdent].datasets[0];
    curConfig.data.datasets[0].trendlineLinear.colorMin = colors[colorIdent].datasets[0];
    ...
    curConfig.data.datasets[1].backgroundColor = colors[colorIdent].datasets[1];
